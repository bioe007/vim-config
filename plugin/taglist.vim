...
function! s:Tlist_Window_Toggle_Help_Text()
    ...
    if s:tlist_brief_help
        ...
        call s:Tlist_Window_Update_Line_Offsets(0, 1, full_help_size -
                    \ brief_help_size)
    else
        let s:tlist_brief_help = 1
        ...
        " Adjust the start/end line numbers for the files
        call s:Tlist_Window_Update_Line_Offsets(0, 0, full_help_size -
                    \ brief_help_size)
    endif
...
function! s:Tlist_Window_Get_File_Index_By_Linenum(lnum)
    call s:Tlist_Log_Msg(
                \'Tlist_Window_Get_File_Index_By_Linenum (' . a:lnum . ')')
...
function! s:Tlist_Window_Init()
    ...
    nnoremap <buffer> <silent> <F1> :call
                \ <SID>Tlist_Window_Toggle_Help_Text()<CR>
    ...
    inoremap <buffer> <silent> [[
                \ <C-o>:call <SID>Tlist_Window_Move_To_File(-1)<CR>
    ...
    inoremap <buffer> <silent> <F1>
                \ <C-o>:call <SID>Tlist_Window_Toggle_Help_Text()<CR>
...
function! s:Tlist_Refresh()
    ...
    if tlist_win != -1
        ...
        if g:Tlist_Highlight_Tag_On_BufEnter && g:Tlist_Auto_Highlight_Tag
            ...
            " Highlight the current tag
            call s:Tlist_Window_Highlight_Tag(filename, cur_lnum, 1,
                        \ center_tag_line)
        else
            " Restore the cursor position
            if v:version >= 601
...
function! s:Tlist_Menu_File_Refresh(fidx)
    call s:Tlist_Log_Msg('Refreshing the tag menu for ' .
                \ s:tlist_{a:fidx}_filename)
